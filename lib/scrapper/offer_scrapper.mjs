...
async function scrapeInstagramOffers(username, tag = 'oferta') {
    ...
    const links = await page.$$eval("a[href^='/p/']", els => els.map(e => e.href));
    ...
    for (const postUrl of links) {
        ...
        const imagesPath = await page.$$eval('article img', imgs =>
...
async function saveOffersInSupabase(offers) {
